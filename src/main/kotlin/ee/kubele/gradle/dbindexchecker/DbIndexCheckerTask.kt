...
abstract class DbIndexCheckerTask : DefaultTask() {
	...
	@Option(
		...
		description = "Write current missing indexes into baseline JSON and exit without failing"
	)
	...
	private fun finish(allMissing: List<MissingIndex>, checkedServices: Set<String>, rootDir: File) {
		...
		if (shouldWriteBaseline) {
			...
			ReportGenerator.generate(
				comparison,
				logger,
				htmlReportFile.get().asFile,
				jsonReportFile.get().asFile,
				warnOnExistingMissing.get()
			)
			return
		}
	...
	private fun discoverModules(rootDir: File): List<String> {
		...
		return rootDir.listFiles()
			...
			?.filter { moduleDir ->
				...
				val hasEntity =
					kotlinSrc.exists() && kotlinSrc.walkTopDown().any { it.isDirectory && it.name in entityNames }
				val hasRepo =
					kotlinSrc.exists() && kotlinSrc.walkTopDown().any { it.isDirectory && it.name in repoNames }
