...
object EntityParser {
	...
	private val TABLE_ANNOTATION = Regex("""@Table\s*\((?:(?:name|value)\s*=\s*)?"([^"]+)"""")
	...
	fun parseEntityFile(file: File): TableMapping? {
		...
		val fieldToColumn = buildMap {
			...
			while (i < lines.size) {
				...
				if (ONE_TO_MANY.containsMatchIn(line) || MANY_TO_MANY.containsMatchIn(line) || JOIN_TABLE.containsMatchIn(
						line
					)
				) {
					i++
				...
				if (columnMatch != null) {
					val columnName = columnMatch.groupValues[1].ifEmpty { columnMatch.groupValues[2] }
					findFieldName(lines, i)?.let { put(it, columnName) }
					i++
					continue
				}
