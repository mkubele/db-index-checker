...
object LiquibaseParser {
	...
	private fun parseXmlChangelogFile(
		...
		forEachElement(doc, "include") { includeElem ->
			...
			if (includePath.isNotBlank()) {
				parseChangelogFile(
					resolveLiquibasePath(includePath, parentDir, liquibaseDir),
					liquibaseDir,
					results,
					visited
				)
	...
	private fun parseSqlChangelogFile(
		...
		includeSqlRegex.findAll(sqlText).forEach { match ->
			...
			parseChangelogFile(
				resolveLiquibasePath(includePath, file.parentFile, liquibaseDir),
				liquibaseDir,
				results,
				visited
			)
	 ...
	 */
	private fun parseCreateTableUniqueConstraints(
		doc: Document,
		filePath: String,
		results: MutableList<IndexedColumn>
	) {
