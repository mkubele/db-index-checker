...
object ReportGenerator {
	...
	private val MISSING_COMPARATOR = compareBy<MissingIndex>(
		{ it.serviceName.lowercase() },
		{ it.tableName.lowercase() },
		{ it.columnName.lowercase() }
	)
	private val ISSUE_COMPARATOR = compareBy<BaselineIssue>(
		{ it.serviceName.lowercase() },
		{ it.tableName.lowercase() },
		{ it.columnName.lowercase() }
	)
				...
				.forEach { (serviceName, serviceIssues) ->
					...
					serviceIssues
						...
						.forEach { (tableName, tableIssues) ->
							val columns = tableIssues.map { it.columnName }.sortedWith(String.CASE_INSENSITIVE_ORDER)
								.joinToString(", ")
				...
				serviceIssues
					...
					.forEach { (tableName, tableIssues) ->
						...
						tableIssues
							...
							.forEach { issue ->
								val message =
									"        - Column '${issue.columnName}' used in ${formatQueryDescription(issue)} (${issue.repositoryFile}:${issue.lineNumber})"
	...
	private fun StringBuilder.appendIssueTable(title: String, issues: List<MissingIndex>) {
		...
		for (issue in issues.sortedWith(MISSING_COMPARATOR)) {
			...
			appendLine(
				"  <td><a href=\"#\" onclick=\"openInIde('${escapeJs(issue.repositoryFile)}', ${issue.lineNumber}); return false;\">${
					escapeHtml(
						shortPath(issue.repositoryFile)
					)
				}:${issue.lineNumber}</a></td>"
			)
			appendLine("</tr>")
		}
	...
	private fun writeJson(comparison: BaselineComparison, file: File) {
		...
		file.writeText(buildString {
			...
			comparison.resolvedIssues.forEachIndexed { i, issue ->
				...
				appendLine(
					"      {\"service\": ${jsonStr(issue.serviceName)}, \"table\": ${jsonStr(issue.tableName)}, \"column\": ${
						jsonStr(
							issue.columnName
						)
					}}$comma"
