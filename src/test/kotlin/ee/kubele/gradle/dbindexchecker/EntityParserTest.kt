...
class EntityParserTest : TestBase() {
	...
	@Test
	fun `parseEntityFile returns null for file without Table annotation`() {
	...
	@Test
	fun `parseEntityFile parses R2DBC entity with positional Table annotation`() {
		val file = createTempFile()
		try {
			file.writeText(
				"""
			...
			assertEquals("UserEntity", result.entityName)
			assertEquals("user", result.tableName)
			assertEquals("password", result.fieldToColumn["password"])
			assertEquals("email", result.fieldToColumn["email"])
		} finally {
			file.delete()
		}
	}
	...
	@Test
	fun `parseEntityFile parses R2DBC entity with positional Column annotation on separate line`() {
		val file = createTempFile()
		try {
			file.writeText(
				"""
			...
			val result = EntityParser.parseEntityFile(file)
			assertNotNull(result)
			assertEquals("pass_word", result.fieldToColumn["password"])
		} finally {
			file.delete()
		}
	}
